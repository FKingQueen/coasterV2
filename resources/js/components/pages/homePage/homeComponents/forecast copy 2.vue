<template>
    <div class="">
                        ...
                        <div class="w-2/4 flex items-center">
                            <table class="w-full border-none">
                                <tr class="border-b">
                                    ...
                                    <td class="py-1">{{ currentWeatherData.currentWeatherWindDirection }} {{
                                        currentWeatherData.currentWeatherWind }} km/h</td>
                                </tr>
                                <tr class="border-b">
        ...
        <highcharts :options="chartOptions" :callback="onChartInit" ref="chartRef" v-else class="hc w-full pt-2 drop-shadow-lg">
        </highcharts>
    </div>
</template>
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            chartOptions: {
                chart: {
                    events: {
                        load: () => {  // Use arrow function to preserve 'this' context
                            console.log('Chart loaded successfully');
                            console.log(this.chartInstance, this.$refs.chartRef.chart);
                            // this.createChart(); // 'this' now refers to your Vue component
                        }
                    }
                },
                title: {
    ...
    methods: {
        ...
        drawWeatherSymbols(chartOptions) {
            this.clearWeatherSymbols();

            if (!chartOptions || !chartOptions.renderer) {
                console.error('Chart or renderer not available')
                return
            }
            chartOptions.series[0].data.forEach((point, i) => {
                ...
                const icon = noDayNightSymbols.includes(symbol)
                    ? symbol // Use the symbol as is
                    : symbol + (isDayTime ? 'd' : 'n');
                const iconRef = chartOptions.renderer
                    .image(
                        `https://nrkno.github.io/yr-weather-symbols/symbols/shadows/${icon}.svg`,
                        point.plotX + chartOptions.plotLeft - 8,
                        point.plotY + chartOptions.plotTop - 30,
                        30,
                        30
                    )
                    .attr({
        ...
        drawBlocksForWindArrows(chartOptions) {
            ...
            for (
                ...
                chartOptions.renderer
                    .path([
                        'M', x, chartOptions.plotTop + chartOptions.plotHeight + (isLong ? 0 : 28),
                        'L', x, chartOptions.plotTop + chartOptions.plotHeight + 32,
                        'Z'
                    ])
                    .attr({
                        stroke: chartOptions.options.chartOptions.plotBorderColor,
                        'stroke-width': 1
                    })
                    .add()
            }

            chartOptions.get('windbarbs').markerGroup.attr({
                translateX: chartOptions.get('windbarbs').markerGroup.translateX + 8
            })
        },


        createChart() {
            // this.chartInstance = this.chartOptions
            if (!this.chartInstance) {
